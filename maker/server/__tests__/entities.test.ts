...
describe('Loot Tables CRUD', () => {
  const ltData = {
    id: 'test_goblin_loot',
    items: '[{"itemId":"gold_coin","chance":0.5}]',
    coinDrop: '{"min":5,"max":10}',
  }

  it('POST /api/loot-tables creates', async () => {
    const res = await request(app).post('/api/loot-tables').send(ltData)
    expect(res.status).toBe(200)
    expect(res.body.id).toBe('test_goblin_loot')
  })

  it('GET /api/loot-tables returns list', async () => {
    const res = await request(app).get('/api/loot-tables')
    expect(res.body).toHaveLength(1)
  })

  it('GET /api/loot-tables/:id returns table', async () => {
    const res = await request(app).get('/api/loot-tables/test_goblin_loot')
    expect(res.status).toBe(200)
  })

  it('PUT /api/loot-tables/:id updates', async () => {
    const res = await request(app)
      .put('/api/loot-tables/test_goblin_loot')
      .send({ coinDrop: '{"min":10,"max":20}' })
    expect(res.status).toBe(200)
    expect(res.body.coinDrop).toBe('{"min":10,"max":20}')
  })

  it('DELETE /api/loot-tables/:id deletes', async () => {
    const res = await request(app).delete('/api/loot-tables/test_goblin_loot')
    expect(res.status).toBe(200)
    const list = await request(app).get('/api/loot-tables')
    expect(list.body).toHaveLength(0)
  })
})
